...
require __DIR__.'/../src/ResponseHeader.php';
...
$c=new Cookie('SID');testEqual($c->setValue('31d4d96e407aad42')->setSameSiteRestriction('Strict'), 'Set-Cookie: SID=31d4d96e407aad42; path=/; httponly; SameSite=Strict');
$c=new Cookie('SID');testEqual($c->setValue('31d4d96e407aad42')->setDomain('localhost')->setSameSiteRestriction('Strict'), 'Set-Cookie: SID=31d4d96e407aad42; path=/; httponly; SameSite=Strict');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('localhost'), 'Set-Cookie: key=value; path=/; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('.localhost'), 'Set-Cookie: key=value; path=/; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('127.0.0.1'), 'Set-Cookie: key=value; path=/; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('.local'), 'Set-Cookie: key=value; path=/; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('example.com'), 'Set-Cookie: key=value; path=/; domain=.example.com; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('.example.com'), 'Set-Cookie: key=value; path=/; domain=.example.com; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('www.example.com'), 'Set-Cookie: key=value; path=/; domain=.www.example.com; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('.www.example.com'), 'Set-Cookie: key=value; path=/; domain=.www.example.com; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('blog.example.com'), 'Set-Cookie: key=value; path=/; domain=.blog.example.com; httponly; SameSite=Lax');
$c=new Cookie('key');testEqual($c->setValue('value')->setDomain('.blog.example.com'), 'Set-Cookie: key=value; path=/; domain=.blog.example.com; httponly; SameSite=Lax');

setcookie('hello', 'world', time() + 86400, '/foo/', 'example.com', true, true);
$cookie = Cookie::parse(ResponseHeader::take('Set-Cookie'));

$c=new Cookie('hello');testEqual($cookie, $c->setValue('world')->setMaxAge(86400)->setPath('/foo/')->setDomain('example.com')->setHttpOnly(true)->setSecureOnly(true));
...
($cookie->getSameSiteRestriction() === Cookie::SAME_SITE_RESTRICTION_LAX) or fail(__LINE__);

testEqual(Cookie::exists('SESSID'), isset($_COOKIE['SESSID']));
testEqual(Cookie::exists('other'), isset($_COOKIE['other']));
testEqual(Cookie::get('SESSID'), (isset($_COOKIE['SESSID']) ? $_COOKIE['SESSID'] : null));
testEqual(Cookie::get('SESSID', 42), (isset($_COOKIE['SESSID']) ? $_COOKIE['SESSID'] : 42));
testEqual(Cookie::get('other'), (isset($_COOKIE['other']) ? $_COOKIE['other'] : null));
testEqual(Cookie::get('other', 42), (isset($_COOKIE['other']) ? $_COOKIE['other'] : 42));
...
function testCookie($name, $value = null, $expire = 0, $path = null, $domain = null, $secure = false, $httpOnly = false) {
	$actualValue = Cookie::buildCookieHeader($name, $value, $expire, $path, $domain, $secure, $httpOnly);
...
function simulateSetCookie($name, $value = null, $expire = 0, $path = null, $domain = null, $secure = false, $httpOnly = false) {
	...
	return ResponseHeader::take('Set-Cookie');
}
